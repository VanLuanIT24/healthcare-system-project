...
export default function ReceptionistPatientRecordsDetailPage() {
  ...
  if (loading) {
    return (
      <ReceptionistShell title="Chi tiết hồ sơ bệnh nhân" activeSection="patientRecords">
  ...
  if (error || !patient) {
    return (
      <ReceptionistShell title="Chi tiết hồ sơ bệnh nhân" activeSection="patientRecords">
        <div className="rd-content" style={{ textAlign: 'center', padding: '40px' }}>
          <div style={{ marginBottom: '16px', color: '#d63b4f' }}>{error || 'Không tìm thấy bệnh nhân.'}</div>
          <button
            type="button"
            onClick={() => navigate('/receptionist/patient-records')}
            style={{
              padding: '8px 16px',
              borderRadius: '6px',
              border: 'none',
