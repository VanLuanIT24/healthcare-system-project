...
export function useFinanceFilters(initial = {}) {
...
export function useFinanceReport(loader, initial = {}) {
  const { filters, setFilters, resetFilters } = useFinanceFilters(initial);
  ...
  const load = useCallback(async ({ silent = false } = {}) => {
    ...
    try {
      ...
      setError({
        status,
        message: status === 403 ? 'Bạn không có quyền xem báo cáo này.' : requestError?.message || 'Không thể tải báo cáo tài chính.',
      });
    } finally {
