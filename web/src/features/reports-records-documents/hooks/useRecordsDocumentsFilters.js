...
export function useRecordsDocumentsFilters(initial = {}) {
...
export function useRecordsDocumentsReport(loader, initial = {}) {
  const { filters, setFilters, resetFilters } = useRecordsDocumentsFilters(initial);
  ...
  const load = useCallback(async ({ silent = false } = {}) => {
    ...
    try {
      ...
      setError({
        status,
        message: status === 403
          ? 'Bạn không có quyền xem báo cáo hồ sơ/tài liệu này.'
          : requestError?.message || 'Không thể tải báo cáo hồ sơ/tài liệu.',
      });
    } finally {
