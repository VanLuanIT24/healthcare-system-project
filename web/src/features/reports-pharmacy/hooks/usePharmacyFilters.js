...
export function usePharmacyFilters(initial = {}) {
...
export function usePharmacyReport(loader, initial = {}) {
  const { filters, setFilters, resetFilters } = usePharmacyFilters(initial);
  ...
  const load = useCallback(async ({ silent = false } = {}) => {
    ...
    try {
      ...
      setError({
        status,
        message: status === 403 ? 'Bạn không có quyền xem báo cáo dược này.' : requestError?.message || 'Không thể tải báo cáo dược.',
      });
    } finally {
