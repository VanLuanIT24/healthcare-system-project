...
export function useDiagnosticsFilters(initial = {}) {
...
export function useDiagnosticsReport(loader, initial = {}) {
  const { filters, setFilters, resetFilters } = useDiagnosticsFilters(initial);
  ...
  const load = useCallback(async ({ silent = false } = {}) => {
    ...
    try {
      ...
      setError({
        status,
        message: status === 403 ? 'Bạn không có quyền xem báo cáo này.' : requestError?.message || 'Không thể tải báo cáo cận lâm sàng.',
      });
    } finally {
