...
export function useInpatientEmergencyFilters(initial = {}) {
...
export function useInpatientEmergencyReport(loader, initial = {}) {
  const { filters, setFilters, resetFilters } = useInpatientEmergencyFilters(initial);
  ...
  const load = useCallback(async ({ silent = false } = {}) => {
    ...
    try {
      ...
      setError({
        status,
        message: status === 403 ? 'Bạn không có quyền xem báo cáo nội trú/cấp cứu này.' : requestError?.message || 'Không thể tải báo cáo nội trú/cấp cứu.',
      });
    } finally {
