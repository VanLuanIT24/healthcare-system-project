...
function authorize({ roles = [], permissions = [], allPermissions = [], anyPermissions = [], actorTypes = [] } = {}) {
  return function authorizeMiddleware(req, res, next) {
    ...
    if (!requireActorType(req.auth, actorTypes)) {
      return next(ApiError.forbidden('Loại tài khoản này không được phép thực hiện chức năng này.'));
    }

    if (roles.length > 0) {
      const hasAllowedRole = req.auth.roles.some((role) => roles.includes(role));
      if (!hasAllowedRole) {
        return next(ApiError.forbidden('Vai trò hiện tại không được phép thực hiện chức năng này.'));
      }
    }

    if (permissions.length > 0 && !hasAllPermissions(req.auth, permissions)) {
      return next(ApiError.forbidden('Tài khoản hiện tại không có quyền truy cập chức năng này.'));
    }

    if (allPermissions.length > 0 && !hasAllPermissions(req.auth, allPermissions)) {
      return next(ApiError.forbidden('Tài khoản hiện tại chưa có đủ tất cả quyền yêu cầu.'));
    }
