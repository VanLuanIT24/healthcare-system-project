...
const MOJIBAKE_PATTERN = /(?:[ÃÂÄÆ]|á[»º]|Â·|�)/;
...
function encodeWindows1252Bytes(value) {
  ...
  for (const char of value) {
    ...
    } else if (CP1252_BYTE_BY_CODE_POINT.has(codePoint)) {
      bytes.push(CP1252_BYTE_BY_CODE_POINT.get(codePoint));
    } else {
...
function repairMojibakeText(value) {
  ...
  const repaired = encodeWindows1252Bytes(text).toString('utf8');
...
const env = {
  ...
  chatbot: {
    ...
    botDisplayName: parseText(process.env.CHATBOT_BOT_DISPLAY_NAME, 'Trợ lý tư vấn & đặt lịch'),
    ...
    handoffFirstResponseTimeoutMinutes: parsePositiveNumber(process.env.CHATBOT_HANDOFF_FIRST_RESPONSE_TIMEOUT_MINUTES, 2),
    handoffMaxEscalationStaff: parsePositiveNumber(process.env.CHATBOT_HANDOFF_MAX_ESCALATION_STAFF, 2),
    handoffPrimaryRoleCodes: parseList(process.env.CHATBOT_HANDOFF_PRIMARY_ROLE_CODES || 'receptionist,scheduler'),
    handoffFallbackRoleCodes: parseList(process.env.CHATBOT_HANDOFF_FALLBACK_ROLE_CODES || 'manager,admin,super_admin'),
